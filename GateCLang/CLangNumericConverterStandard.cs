...
namespace Gate.CLanguage
{
   ...
   /// </summary>
   public unsafe class CLangNumericConverterStandard : INumericConverter
   {
      private readonly static List<InnerConversionFunction> myListConversionFunctions = new List<InnerConversionFunction>();
      private readonly static ExtraDllCppCode myCppCodeConversion;
      ...
      public CLangNumericConverterStandard() { }

      static CLangNumericConverterStandard() => myCppCodeConversion = myGetCppCodeForConversion();

      private class InnerConversionFunction
      {
         public InnerConversionFunction(Type outputType, Type inputType)
         {
            InputType = inputType;
         ...
         public ConvertHandler ConvertHandler => myCppCodeConversion.DllInstance.GetDelegate<ConvertHandler>(ProcName);
      ...
      private static ExtraDllCppCode myGetCppCodeForConversion()
      {
         ...
         {
            var cf = new InnerConversionFunction(pai.t1, pai.t2);
         ...
         var cpp_cod = new ExtraDllCppCode("NumericConverterStandard", sb.ToString(), new CompileEnvGcc(), true);
      ...
      public T? DoConvertCsharpValue<T>(ValueType rValue) where T : struct => (T?)DoConvertCsharpValue(typeof(T), rValue);

      public virtual ValueType? DoConvertCsharpValue(Type lType, ValueType rValue) => Convert(lType, rValue);

      public virtual void ConvertPointers(IntPtr outPointer, Type outType, int outStep, IntPtr inPointer, Type inType, int inStep, int n)
      {
         var cf = myListConversionFunctions.FirstOrDefault(f => f.InputType == inType && f.OutputType == outType) ?? throw new Crash();

         cf.ConvertHandler(outPointer, outStep, inPointer, inStep, n);
      }

      public static T? Convert<T>(ValueType inValue) where T : struct => (T?)Convert(typeof(T), inValue);

      public static ValueType? Convert(Type outType, ValueType inValue)
      {
         ...
         {
            var cf = myListConversionFunctions.FirstOrDefault(f => f.InputType == inValue.GetType() && f.OutputType == outType) ?? throw new Crash();
      ...
      /// <returns></returns>
      public static ValueType? GetBitField(ValueType intValue, BitField bitField)
      {
         unchecked
         {
      ...
      /// <param name="bitField"></param>
      /// <returns></returns>
      public static ValueType? UpdateBitField(ValueType intValueIn, ValueType bitFieldToSet, BitField bitField)
      {
         unchecked
         {
